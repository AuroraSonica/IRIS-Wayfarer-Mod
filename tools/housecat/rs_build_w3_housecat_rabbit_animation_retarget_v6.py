"""V6 housecat->rabbit retarget: v5 solver + three surgical fixes.

V5 (correct limb chains, physical-joint mapping, neutral distal paws) remains
the base solver.  V6 changes exactly three things:

1. QUATERNION HEMISPHERE ENFORCEMENT.  V5 keyed each frame's quaternion
   independently; all four turn clips contain one Hip sign flip mid-clip.
   Exact 30 fps frames hide it, but the 60 fps FBX resample and the engine's
   own interpolation sweep through garbage between flipped keys.  V6 keeps
   every baked quaternion in the hemisphere of its previous frame.

2. IN-PLACE LOOPS.  The v1 Hip translation carries source world travel
   (run: 0.65 m per 12-frame cycle).  IRIS Lua owns travel (same law as the
   proven W3 horse port), so v6 subtracts the straight first->last horizontal
   drift line from looping clips, preserving vertical bob and intra-stride
   surge.  One-shot clips keep their authored translation.

3. PATH-FOLLOWING SOLVER FOR ONE-SHOTS.  rest.rotation_difference(live) has
   no stable axis once a segment swings near 180 degrees from rest - exactly
   what forelegs do in the taunt rear-up and the hiss crouch, which is why
   those clips collapsed with crossing legs.  For non-looping clips v6
   composes frame-to-frame direction steps (each step small and stable) so
   the segment follows the source's actual swing path.  Looping clips keep
   the v5 per-frame solve, whose excursions stay in the stable range.
"""
...
OUTPUT = RS / "w3_housecat_rabbit_animation_retarget_v6.blend"
REPORT = RS / "w3_housecat_rabbit_animation_retarget_v6_report.json"
...
report = {
    "format": "iris-w3-housecat-rabbit-retarget-v6",
    "source": str(INPUT),
    "output": str(OUTPUT),
    "method": (
...
for clip in CLIPS:
    ...
    old = bpy.data.actions.get(f"IRIS_HOUSECAT_RABBIT_V6__{clip}")
    if old:
        bpy.data.actions.remove(old)
    action = base_action.copy()
    action.name = f"IRIS_HOUSECAT_RABBIT_V6__{clip}"
    action.use_fake_user = True
    action["iris_retarget_version"] = 6
    is_oneshot = clip in ONESHOT_CLIPS
    action["iris_limb_solver"] = (
        "path-following incremental direction solver" if is_oneshot
        else "complete-chain anatomical direction solver"
    )
    ...
    # incremental path state (one-shots): motion quat + previous live direction
    inc_motion = {}
    inc_prev = {}
    prev_keyed_quat = {}
    ...
    for frame in range(first, last + 1):
        ...
        for source_parent, source_child, target_bone, _target_child in DIRECTION_SEGMENTS:
            ...
            if is_oneshot:
                if target_bone not in inc_motion:
                    inc_motion[target_bone] = source_rest_local[target_bone].rotation_difference(
                        source_live_local
                    )
                else:
                    step = inc_prev[target_bone].rotation_difference(source_live_local)
                    inc_motion[target_bone] = step @ inc_motion[target_bone]
                inc_prev[target_bone] = source_live_local
                motion = inc_motion[target_bone]
            else:
    ...
    report["clips"].append({
        "name": clip,
        "frames": [first, last],
        "mode": "oneshot-incremental" if is_oneshot else "loop-v5-solver",
        "horizontal_drift_removed_m": [round(v, 4) for v in drift_removed],
        "action": action.name,
    })
    print("V6_CLIP", clip, "oneshot" if is_oneshot else "loop",
          "drift", [round(v, 3) for v in drift_removed])

target.animation_data.action = bpy.data.actions["IRIS_HOUSECAT_RABBIT_V6__walk"]
...
print("HOUSECAT_RETARGET_V6_OK", OUTPUT)
print("HOUSECAT_RETARGET_V6_REPORT", REPORT)
