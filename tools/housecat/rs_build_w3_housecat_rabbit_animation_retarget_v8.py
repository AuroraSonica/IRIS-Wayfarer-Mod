"""V8 housecat->rabbit retarget: per-segment IDENTITY/INTENT blend.

The stance mismatch is real (cat rest spine 16 deg of curl, rabbit 44 deg;
cat gait narrow, rabbit front-leg base wide) - it caused the "full body bend"
and "front paws off" field reports against v6.  But v7's pure ABSOLUTE
direction transfer failed on the mesh: the skin is bound at the rabbit rest
pose, and holding the skeleton at a constant large offset from bind crumpled
the body (worst on the head).

V8 blends the two stable solutions per segment:

    desired = slerp(v6_solution, source_absolute_direction, alpha)

with alpha chosen where the field complaints live: spine high (kill the
over-fold), legs medium (narrow the gait most of the way), head/tail handles
zero (v6 looked right there, and alpha=0 sidesteps their loop twist issue).

Kept from V6/V7: hemisphere-consistent quaternion keys, in-place loops,
incremental path solve for one-shots, one-shot handle-bone twist continuity,
per-clip constant ground pin.
"""
...
OUTPUT = RS / "w3_housecat_rabbit_animation_retarget_v8.blend"
REPORT = RS / "w3_housecat_rabbit_animation_retarget_v8_report.json"
...
# Blend toward the source's absolute anatomical direction, per target bone.
# 0.0 = pure v6 rest-relative (rabbit identity, skin nearest bind);
# 1.0 = pure absolute (cat intent, v7's mesh-crumple failure mode).
ALPHA = {
...
for clip in CLIPS:
    ...
    old = bpy.data.actions.get(f"IRIS_HOUSECAT_RABBIT_V8__{clip}")
    if old:
        bpy.data.actions.remove(old)
    action = base_action.copy()
    action.name = f"IRIS_HOUSECAT_RABBIT_V8__{clip}"
    action.use_fake_user = True
    action["iris_retarget_version"] = 8
    ...
    handle_prev_rot = {}
    handle_first_rot = {}
    # incremental path state for one-shot clips (v6 endpoint of the blend)
    inc_motion = {}
    inc_prev = {}
    ...
    for frame in range(first, last + 1):
        ...
        for source_parent, source_child, target_bone, _target_child in DIRECTION_SEGMENTS:
            ...
            # v6 endpoint: rest-relative (incremental on one-shots)
            if is_oneshot:
            ...
            v6_local = (motion @ target_rest_local[target_bone]).normalized()
            # blend toward the source's absolute direction
            alpha = ALPHA[target_bone]
            if alpha <= 0.0:
                desired_local = v6_local
            elif alpha >= 1.0:
                desired_local = source_live_local
            else:
                swing = v6_local.rotation_difference(source_live_local)
                partial = Quaternion(swing.axis, swing.angle * alpha)
                desired_local = (partial @ v6_local).normalized()
    ...
    for bone_name, first_rot in handle_first_rot.items():
        ...
        if clip in LOOP_CLIPS and wrap > 25.0:
            print(f"V8_WRAP_WARN {clip} {bone_name} {wrap:.0f}deg")
...
target.animation_data.action = bpy.data.actions["IRIS_HOUSECAT_RABBIT_V8__walk"]
...
print("HOUSECAT_RETARGET_V8_OK", OUTPUT)
print("HOUSECAT_RETARGET_V8_REPORT", REPORT)
