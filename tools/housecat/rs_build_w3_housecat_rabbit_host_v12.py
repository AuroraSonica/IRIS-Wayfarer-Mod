"""v12 house-cat mesh bench: v11 + targeted NECK relax.

Aurora's v11 field verdict (08-19): holes/spikes GONE, but "still a dent in
around the neck under the head".  The throat sits where the Neck_0 and
Head_0/Spine_3 warp deltas diverge hardest; the generic crease relax
under-treats it.  v12 adds 3 extra smooth passes restricted to the neck
ring (Neck_0 weight > 0.05, Head_0 weight < 0.6 so the skull keeps its
authored shape), followed by the same seam weld.  Everything below is
v11-verbatim.
...
1. CREASE RELAX (bmesh.ops.smooth_vert) smooths by CONNECTIVITY -- seam
   ...
   the seam tears open (the chest/leg holes).  v12 keeps the relax but
...
2. SUBDIV smooth=0.8 pulls the two copies of every seam edge to different
   smoothed midpoints -> more tears.  v12 reverts to smooth=0.0 (v09
...
3. TAIL CURL had a hard cliff at weight 0.25 -- a face spanning a 0.26 vert
   ...
   spikes).  v12 ramps the curl in with a smoothstep over weight 0.2..0.7,
...
OUTPUT_BLEND = RS / "w3_housecat_dd2_rabbit_host_v12.blend"
OUTPUT_REPORT = RS / "w3_housecat_rabbit_host_v12_report.json"
...
NECK_SMOOTH_FACTOR = 0.5         # v12: throat-dent relax (neck ring only)
NECK_SMOOTH_PASSES = 3
NECK_MIN_W = 0.05                # Neck_0 weight that admits a vert to the ring
NECK_HEAD_CAP = 0.6              # skull-dominated verts keep authored shape
SUBDIV_SMOOTH = 0.0              # v12: BACK to linear -- 0.8 tore every seam
SEAM_WELD_DECIMALS = 5           # v12: coincidence key for the seam weld
TAIL_CURL = math.radians(42.0)
TAIL_RAMP_LO = 0.2               # v12: smoothstep ramp replaces the 0.25 cliff
TAIL_RAMP_HI = 0.7
...
report = {"format": "iris-w3-housecat-rabbit-host-v12", "error": None}
try:
    ...
    # ---- v12: capture seam-duplicate groups BEFORE any smoothing op --------
    ...
    welded = weld_groups(fit, seams)   # v12: close the tears the relax opened

    # ---- v12: targeted neck-ring relax (throat dent under the head) -------
    ring_weights = read_weights(fit)
    neck_ring = [
        vertex.index for vertex in fit.data.vertices
        if ring_weights[vertex.index].get("Neck_0", 0.0) > NECK_MIN_W
        and ring_weights[vertex.index].get("Head_0", 0.0) < NECK_HEAD_CAP
    ]
    bm = bmesh.new()
    bm.from_mesh(fit.data)
    bm.verts.ensure_lookup_table()
    neck_verts = [bm.verts[i] for i in neck_ring]
    for _ in range(NECK_SMOOTH_PASSES):
        bmesh.ops.smooth_vert(
            bm, verts=neck_verts, factor=NECK_SMOOTH_FACTOR,
            use_axis_x=True, use_axis_y=True, use_axis_z=True,
        )
    bm.to_mesh(fit.data)
    bm.free()
    fit.data.update()
    neck_welded = weld_groups(fit, seams)

    # ---- tail curl bake (v12: smoothstep ramp, no cliff) ------------------
    ...
    # ---- subdivision (v12: LINEAR again -- smoothing here tears seams) ----
    bm = bmesh.new()
    bm.from_mesh(fit.data)
    ...
    report.update(
        ...
        neck_ring_vertices=len(neck_ring),
        neck_seam_groups_welded=neck_welded,
