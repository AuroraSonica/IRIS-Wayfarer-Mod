"""V09 house-cat mesh bench: corrected shoulder binding + deformability pass.

Fixes over the v1 host / v08 shape chain:

1. SHOULDER BINDING.  ch99_200's named clavicle helpers sit on the OPPOSITE
   physical side from their upper legs (L_Shoulder_Clavicle x=-0.079 vs
   L_FrontLeg_Upper x=+0.062) and the animation pipeline holds them neutral
   forever.  The v1 host mapped cat l/r_shoulder onto them anyway, which
   (a) warped chest vertices toward the wrong side of the body and
   (b) left the shoulder mass bound to a dead bone, disconnecting the chest
   from the animated front legs.  V09 maps l/r_shoulder onto the physical
   L/R_FrontLeg_Upper for both the warp and the weights.

2. V08 PAW PASS retained: front-paw flattening + distal weight
   redistribution, unchanged fractions.

3. DEFORMABILITY (the W3 horse recipe): 1,298 verts cannot bend without hard
   creases.  One simple subdivision (-> ~5.2k verts), imported custom split
   normals cleared (subdivision turns them to garbage), smooth shading,
   numpy spatial weight smoothing, then cross-side leg-weight strip and
   renormalisation (smoothing reintroduces bleed).
"""
...
OUTPUT_BLEND = RS / "w3_housecat_dd2_rabbit_host_v09.blend"
OUTPUT_REPORT = RS / "w3_housecat_rabbit_host_v09_report.json"
...
MAP = {
    ...
    "tail4": "Tail",
    "tail5": "Tail",
    # V09: the cat scapula rides the animated front leg, never the rabbit's
    # cross-sided, permanently-neutral clavicle helpers.
...
WARP_DRIVER = {
    ...
    "tail4": "tail1",
    "tail5": "tail1",
    # anchor the scapula warp on the bicep joint so shoulder verts keep their
    # authored offset from the leg root instead of collapsing onto it
    "l_shoulder": "l_bicep",
    "r_shoulder": "r_bicep",
}

# v08 paw pass, unchanged
TRANSFERS = (
...
report = {"format": "iris-w3-housecat-rabbit-host-v09", "error": None}
try:
    ...
    bmesh.ops.subdivide_edges(
        bm, edges=bm.edges[:], cuts=1, use_grid_fill=True, smooth=0.0
    )
    bm.to_mesh(fit.data)
    bm.free()
    fit.data.update()
    ...
    # ---- numpy spatial weight smoothing ----------------------------------
    ...
    for i in range(count):
        ...
        total = row.sum()
        if total <= 1e-8:
            # fall back to the nearest weighted neighbour's dominant bone
            nb = neighbours[i]
    ...
    report.update(
        ...
        shoulder_map="l/r_shoulder -> L/R_FrontLeg_Upper (v09 fix)",
        paw_shape=paw_shape,
