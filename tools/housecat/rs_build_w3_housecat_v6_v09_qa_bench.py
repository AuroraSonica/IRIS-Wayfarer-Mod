...
RS = ROOT / "rs_tools"
BENCH = RS / "w3_housecat_rabbit_animation_retarget_v6.blend"
V09 = RS / "w3_housecat_dd2_rabbit_host_v09.blend"
QA_BLEND = RS / "w3_housecat_v6_v09_qa.blend"
OUT = RS / "housecat_qa_v6_v09"
...
for clip, (frames, views) in RENDERS.items():
    act = bpy.data.actions.get(f"IRIS_HOUSECAT_RABBIT_V6__{clip}")
    if act is None:
        print("SKIP", clip)
        continue
