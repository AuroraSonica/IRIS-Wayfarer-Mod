"""v0.9.2 DISCRIMINATOR pak: the proven v0.8 pak byte-verbatim + ONLY the
W3 motlist appended.
...
but offline validation cannot prove mesh content either way.  v0.9.2 keeps
...
FLUFFY = RS / "w3_housecat_fluffy_v0.9.2"
PAK = FLUFFY / "IRIS_10_housecat_v092.pak"
ZIP = RS / "IRIS_HouseCat_Prototype_v0.9.2_FluffyMod.zip"
...
motlist_raw = (STAGE / MOTLIST_NAME).read_bytes()
entries.append((hash_utf16(MOTLIST_NAME.lower()), hash_utf16(MOTLIST_NAME.upper()),
                deflate(motlist_raw), len(motlist_raw), 1, 0))

new_names = [n for n in manifest_names if n.upper() != MANIFEST_NAME.upper()] + [MOTLIST_NAME]
...
version=0.9.2-discriminator-old-body-plus-motlist
...
""", encoding="utf-8")
(FLUFFY / "README.txt").write_text("""IRIS House Cat Prototype v0.9.2 (discriminator)
===============================================
